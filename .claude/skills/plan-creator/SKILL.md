...
- **`plan_output_dir`**: Where to save plans (default: `docs/plans/`)
- **`plan_filename_format`**: Template for plan filenames (default: `YYYY-MM-DD-description.md`)
- **`sync_command`**: Optional command to sync plans (e.g., `git add docs/plans`)
- **`verification_commands`**: Default verification commands (e.g., `make test`, `npm run lint`)
- **`input_docs_dir`**: Optional directory for ticket/requirement files (e.g., `docs/tickets/`)
...
1. **Read all mentioned files immediately and FULLY**:
   - Ticket files (e.g., `docs/tickets/feature-123.md`)
   - Research documents
...
2. **Spawn initial research tasks to gather context**:
   ...
   - If relevant, use knowledge base agents (e.g., **thoughts-locator**) to find existing documents about this feature
...
3. **Spawn parallel sub-tasks for comprehensive research**:
   ...
   **For deeper investigation:**
   ...
   - **codebase-analyzer** - To understand implementation details (e.g., "analyse how [system] works")
   - **codebase-pattern-finder** - To find similar features we can model after

   **For historical context:**
   - **thoughts-locator** (or equivalent) - To find any research, plans, or decisions about this area
   - **thoughts-analyzer** (or equivalent) - To extract key insights from the most relevant documents
...
1. **Write the plan** to configured `plan_output_dir`
   - Format filename using `plan_filename_format` template
   - Default: `YYYY-MM-DD-description.md` where:
     - YYYY-MM-DD is today's date
     - description is a brief kebab-case description
   - Examples:
     - `2025-10-27-add-authentication.md`
     - `2025-10-27-refactor-api-layer.md`
...
1. **Sync the plan** (if `sync_command` configured):
...
3. **Iterate based on feedback** - be ready to:
   ...
   - After making changes, sync again if configured
